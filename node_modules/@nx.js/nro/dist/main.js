...
const nxjsNroUrl = new URL(isSrcMode ? '../../../nxjs.nro' : '../dist/nxjs.nro', import.meta.url);
...
try {
    ...
    console.log(chalk.yellow(`⚠️  No ${chalk.bold(`"${iconName}"`)} file found. Default nx.js icon will be used.`));
}
...
function walk(dir, dirEntry) {
    for (const name of readdirSync(dir)) {
        ...
        if (stat.isDirectory()) {
            ...
            console.log(`  ${chalk.cyan(relative(romfsDirPath, fileURLToPath(fileUrl)))} (${bytes(blob.size).toLowerCase()})`);
            dirEntry[name] = blob;
        }
        else {
...
if (!(romfs['main.js'] instanceof Blob)) {
    console.log();
    console.log(chalk.yellow(`${chalk.bold('Warning!')} No "main.js" file found in \`romfs\` directory.`));
    console.log(chalk.yellow(`The entrypoint file ${chalk.bold(`"${nacp.title}.js"`)} will need to`));
    console.log(chalk.yellow(`be placed alongside ${chalk.bold(`"${outputNroName}"`)} on the SD card.`));
}
...
console.log(`${chalk.green(`\n🎉 Success! Generated NRO file ${chalk.bold(`"${outputNroName}"`)}`)} (${bytes(outputNro.size).toLowerCase()})`);
