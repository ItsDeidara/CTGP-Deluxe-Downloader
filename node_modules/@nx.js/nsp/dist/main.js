...
import { cpSync, existsSync, rmSync, mkdtempSync, readFileSync, writeFileSync, readdirSync, statSync, } from 'node:fs';
...
const createTmpDir = (type) => {
    const dir = new URL(`${pathToFileURL(mkdtempSync(join(tmpDir, `nxjs-nsp_${type}.`)))}/`);
    tmpPaths.push(dir);
    return dir;
};
...
try {
    ...
    console.log(await terminalImage.file(fileURLToPath(iconPath), { height: 12 }));
    // NACP
    ...
    if (signal) {
        ...
        console.log(chalk.green(`\n🎉 Success! Generated NSP file ${chalk.bold(`"${outputNspName}"`)}`) + ` (${bytes(statSync(nspDest).size).toLowerCase()})`);
    }
}
finally {
